...
class Nueva_Satisfaccion_sibib extends CI_Controller {
...
/** Propiedades **/
	public function set_id( $id ) { $this->id_evaluacion = 7; }
...
/** Constructor **/
	function __construct() {
		...
		if( !$this->session->userdata( 'id_usuario' ) ) {
			...
			$this->set_id( 7 );
			$this->set_nom( 'nueva_satisfaccion_sibib' );
...
/** Funciones **/
	...
	function presentacion() {
		...
		if( $consulta->num_rows() > 0 ) {
			...
			foreach( $consulta->result() as $row ) {
				...
				if( $preguntas->num_rows() > $respuestas->num_rows() ) {
					...
					$main .= '<th width="15"><a href="'.base_url().'index.php/evaluaciones/nueva_satisfaccion_sibib/contestar/'.$row->IdBiblioteca.'/'.$idv.'" onmouseover="ddrivetip(\'Click para alimentar<br />los datos de esta biblioteca\')" onmouseout="hideddrivetip()"><img src="'.base_url().'includes/img/icons/seguimiento.png" /></a></th><td><a href="'.base_url().'index.php/evaluaciones/nueva_satisfaccion_sibib/contestar/'.$row->IdBiblioteca.'/'.$idv.'" onmouseover="tip(\'Click para alimentar<br />los datos de esta biblioteca\')" onmouseout="cierra_tip()" style="color:#333">'.$row->Biblioteca.'</a></td></tr>';
				}
			}
			...
			if( !$resp ) {
				...
				redirect('evaluaciones/nueva_satisfaccion_sibib/index/termino');
			}
			...
			// si no debe evaluar
			if( !$resp )
				redirect('evaluaciones/nueva_satisfaccion_sibib');
		}
		$datos['main'] = $main;
		...
		$this->load->view('_estructura/header',$datos);
		$this->load->view('_estructura/top',$datos);
		$this->load->view('evaluaciones/nueva_satisfaccion_sibib/preguntas',$datos);
		$this->load->view('_estructura/right');
		$this->load->view('_estructura/footer');
	}
			...
			if( $secciones->num_rows() > 0 ) {
				...
				if( $val ) {
					...
					if( $resp ) {
						...
						$datos['enlace'] = "evaluaciones/nueva_satisfaccion_sibib/presentacion";
	...
	function resultados() {
		...
		if( $_POST ) {
			...
			$this->load->view('_estructura/header',$datos);
			$this->load->view('_estructura/top',$datos);
			$this->load->view('evaluaciones/comun/snresultados',$datos);
			$this->load->view('_estructura/right');
			$this->load->view('_estructura/footer');
		}
	}
	...
	// Resultados Generales( todas las bibliotecas y todas las secciones)
		if( $ida == 'all' && $ids == 'all' ) {
			...
			$tags_opciones = array('MDA','DA','ED','MED');
	...
	// Resultados de un biblioteca y de una sección
		...
		$this->load->view('_estructura/header',$datos);
		$this->load->view('_estructura/top',$datos);
		$this->load->view('evaluaciones/nueva_satisfaccion_sibib/grafica',$datos);
		$this->load->view('_estructura/right');
		$this->load->view('_estructura/footer');
	}
}
