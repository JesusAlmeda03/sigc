...
class Nueva_satisfaccion extends CI_Controller {
...
/** Propiedades **/
	public function set_id($id) { $this->id_evaluacion = 6; }
...
/** Constructor **/
	function __construct() {
		...
		if( !$this->session->userdata( 'id_usuario' ) ) {
			...
			$this->set_id( 6 );
			$this->set_nom( 'nueva_satisfaccion' );
...
/** Funciones **/
	...
	function presentacion() {
			...
			case 0 :
				...
				if( $sec->num_rows() > 0 ) {
					...
					foreach( $sec->result() as $row ) {
						$main .= '<tr><td width="15"><a href="'.base_url().'index.php/evaluaciones/nsatisfaccion/contestar/'.$row->IdSeccion.'/'.$idv.'" onmouseover="tip(\'Click para alimentar<br />los datos de estos usuarios\')" onmouseout="cierra_tip()"><img src="'.base_url().'includes/img/icons/seguimiento.png" /></a></td>';
						$main .= '<td><a href="'.base_url().'index.php/evaluaciones/nueva_satisfaccion/contestar/'.$row->IdSeccion.'/'.$idv.'" onmouseover="tip(\'Click para alimentar<br />los datos de estos usuarios\')" onmouseout="cierra_tip()" style="color:#333">'.$row->Seccion.'</a></td></tr>';
					}
					$main .= '</table>';
				}
				break;
			...
			case 1 :
				/*foreach( $resultados->result() as $row_r ) {
					...
					if( $sec->num_rows() > 0 ) {
						...
						foreach( $sec->result() as $row ) {
							$main .= '<tr><td width="15"><a href="'.base_url().'index.php/evaluaciones/nsatisfaccion/contestar/'.$row->IdSeccion.'/'.$idv.'" onmouseover="tip(\'Click para alimentar<br />los datos de estos usuarios\')" onmouseout="cierra_tip()"><img src="'.base_url().'includes/img/icons/seguimiento.png" /></a></td>';
							$main .= '<td><a href="'.base_url().'index.php/evaluaciones/nsatisfaccion/contestar/'.$row->IdSeccion.'/'.$idv.'" onmouseover="tip(\'Click para alimentar<br />los datos de estos usuarios\')" onmouseout="cierra_tip()" style="color:#333">'.$row->Seccion.'</a></td></tr>';
						}
						$main .= '</table>';
				}
				}*/
				redirect('evaluaciones/nueva_satisfaccion/index/termino');
				break;
		...
		$this->load->view('_estructura/header',$datos);
		$this->load->view('_estructura/top',$datos);
		$this->load->view('evaluaciones/nueva_satisfaccion/preguntas',$datos);
		$this->load->view('_estructura/right');
		$this->load->view('_estructura/footer');
	}
	...
	function contestar( $ids, $idv ) {
		...
		if( $_POST ) {
			...
			if ($this -> form_validation -> run() == FALSE) {
						...
						foreach( $preguntas->result() as $row_p ) {
							...
							if( $opciones->num_rows() > 0 ) {
								...
								foreach( $opciones->result() as $row_o ) {
									echo " ".$idt.' '.$row_p->IdPregunta.' '.$row_o->IdOpcion;
									$inserta[$i] = array(
				...
				if( $resp ) {
					...
					$datos['enlace'] = "evaluaciones/nueva_satisfaccion/presentacion";
		...
		// estructura de la página (2)
		$this->load->view('evaluaciones/nueva_satisfaccion/contestar',$datos);
		$this->load->view('_estructura/right');
		$this->load->view('_estructura/footer');
	}
	...
	function resultados() {
		...
		$grafica = $this->db->get( 'en_total_satisfaccion_sigc' );
		if( $grafica->num_rows() == 0 ) {
			redirect( 'evaluaciones/nueva_satisfaccion' );
		}
		...
		if( $_POST ) {
			...
			$this->load->view('_estructura/header',$datos);
			$this->load->view('_estructura/top',$datos);
			$this->load->view('evaluaciones/comun/nresultados',$datos);
			$this->load->view('_estructura/right');
			$this->load->view('_estructura/footer');
		}
	}
	...
	// Resultados Generales( todas las áreas y todas las secciones)
		if( $ida == 'all' && $ids == 'all' ) {
			...
			$opc = array('MDA','DA','ED','MED');
			$tags_opciones = array('Muy de Acuerdo','De Acuerdo','En Desacuerdo','Muy En Desacuerdo');
			...
			if( $evaluaciones->num_rows() > 0 ) {
				foreach( $evaluaciones->result() as $row_eva ) {
					$tabla_header_evaluaciones .= '<th colspan="4" style="font-weight:normal; font-size:14px; color:#FFF; text-align:center">'.$row_eva->Nombre.'</th>';
					//areas
							...
							if( $usuarios_totales->num_rows() > 0 ) {
								...
								foreach( $usuarios_totales->result() as $row_usu ) {
									...
									if( $usuarios_totales->num_rows() == 1 ) {
										$tabla_header_evaluaciones = '<th colspan="4">'.$row_eva->Nombre.'</th>';
									}
								}
								/*if( $total_captura == 6 ) {
		...
		if( $ida != 'all' && $ids == 'all') {
			...
			if( $areas->num_rows() > 0 ) {
				foreach( $areas->result() as $row_are ) {
					...
					if( $evaluaciones->num_rows() > 0 ) {
						foreach( $evaluaciones->result() as $row_eva ) {
							$tabla_header_evaluaciones .= '<th colspan="4" style="font-weight:normal; font-size:18px; color:#FFF; text-align:center;">'.$row_eva->Nombre.'</th>';
							// usuarios totales
	...
	// Resultados de un área y de una sección
		...
		$this->load->view('_estructura/header',$datos);
		$this->load->view('_estructura/top',$datos);
		$this->load->view('evaluaciones/nueva_satisfaccion/grafica',$datos);
		$this->load->view('_estructura/right');
		$this->load->view('_estructura/footer');
	}
}
