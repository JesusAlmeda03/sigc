...
/** Funciones **/	
	...
	function index() {
		...
		$datos['jefes'] = $this->auditoria_model->get_jefes( $this->id );
	...
	function especifico() {
		// variables necesarias para la página
		$titulo = "Programa Espec&iacute;fico de Auditor&iacute;a R8.2.2.1,B";
		$datos['titulo'] = $titulo;
	...
	function lista_verificacion( $tipo ) {
		...
		switch( $tipo ) {
			...
			case 'documento' :
				$this->lista_verificacion_documento( $id_proceso );
				break;
			...
			if( $this->input->post( 'procesos' ) ) {
				...
				// realiza la inserción a la base de datos si todo ha estado bien
					...
					if( $this->auditoria_model->inserta_lista_verificacion( $this->id ) ) {
						...
						$datos['mensaje'] = "Has creado la Lista de Verificaci&oacute;n para el Proceso";
						if( $agregar ) {
	...
	function hallazgos() {
		...
		// obtiene las no conformidades derivadas de los hallazgos
		$datos['conformidades'] = $this->auditoria_model->get_conformidades();
	...
	//
	// lista_verificacion_documento( $id_proceso ): Genera el documento PDF de la lista de verificación
	//
	function lista_verificacion_documento( $id_proceso ) {
		// obtiene la lista de verificación del usuario
		$lista = $this->auditoria_model->get_lista_usuario_proceso( $this->id, $id_proceso );
		...
		// Encabezados
		$this->excel->getActiveSheet()->getStyle('A8:H8')->getFont()->setSize(10);
		...
		$this->excel->getActiveSheet()->getStyle('A8:H8')->getAlignment()->setWrapText(true);
		...
		$this->excel->getActiveSheet()->getColumnDimension('H')->setWidth(3.2);	
		...
		$this->excel->getActiveSheet()->setCellValue('D8', 'Que preguntar');
		$this->excel->getActiveSheet()->setCellValue('E8', 'Observaciones y Descripción de las Evidencias mostradas para cada Cuestionamiento');
		$this->excel->getActiveSheet()->setCellValue('F8', 'C');
		$this->excel->getActiveSheet()->setCellValue('G8', 'NC');
		$this->excel->getActiveSheet()->setCellValue('H8', 'OM');
		...
		if( $lista->num_rows() > 0 ) {
			...
			foreach( $lista->result() as $row ) {
				...
				//Ayuda
				$this->excel->getActiveSheet()->setCellValue('D'.$i, $row->Pregunta);
				$this->excel->getActiveSheet()->getStyle('D'.$i)->getFont()->setSize(10);
	...
	//
	// comprobante( $id_proceso ): Genera y muestra el comprobante de termino de la evaluación
	//
	function comprobante( $id_proceso ) {
		$hallazgos = $this->auditoria_model->get_respuestas_lista( $this->id, $id_proceso );
		$auditoria = $this->auditoria_model->get_auditoria( $this->id );
		/*foreach( $auditoria->result() as $row ) {
			$aud = $row->Auditoria;
			$inicio = $this->inicio_model->set_fecha( $row->Inicio );
			$termino = $this->inicio_model->set_fecha( $row->Termino );
		}*/
		
		$html = '
			<div style="font-family:Arial; padding-bottom:10px; border-top:3px solid #C00; border-bottom:1px solid #CCC">
				<div style="width:120px; text-align:right; float:left; border-top:3px solid #CCC"><img src="'.base_url().'includes/img/logo.jpg" /></div>
				<div style="width:420px; text-align:center; padding:5px 230px 0; float:left; font-weight:bold; font-size:18px; border-top:3px solid #CCC">
					Reporte de Hallazgos y Conclusiones de la Auditoría Interna de Calidad<br />
					R.8.2.2,1,E
				</div>
			</div>
			<br />
			<span style="font-style:italic; font-weight:bold;">'.$auditoria.'</span>
		...
		if( $hallazgos->num_rows() > 0 ) {
			$i = 1;
			$tipo = '';
			foreach( $hallazgos->result() as $row ){
				if( $row->Hallazgo != '' ) {
					if( $tipo == $row->Tipo) {
						$area = $this->auditoria_model->get_area_proceso( $row->IdProceso);
						$html .= '
							<tr>
								<td style="border:1px solid #ccc">'.$i.'</td>
								<td style="border:1px solid #ccc">'.$row->Proceso.'</td>
								<td style="border:1px solid #ccc">'.$row->Requisito.'</td>
								<td style="border:1px solid #ccc">'.$row->Hallazgo.'</td>
								<td style="border:1px solid #ccc">'.$area.'</td>
								<td style="border:1px solid #ccc">'.$this->session->userdata('nombre').'</td>
							</tr>
						';
						$i++;
					}
						...
						$html .= '
							<br />
							<table width="1000" style="font-family:Arial; border:1px solid #CCC;">
								<tr style="background-color:#C00;">
									<th style="color:#FFF; font-size:18px; padding:10px" colspan="6">'.$tipo.'</th>
								</tr>
								<tr style="background-color:#C00;">
									<th style="color:#FFF">#</th>
									<th style="color:#FFF">Proceso</th>
									<th style="color:#FFF">Requerimiento / Punto de la Norma</th>
									<th style="color:#FFF">Descripción</th>
									<th style="color:#FFF">Área</th>
									<th style="color:#FFF">Auditor</th>
								</tr>
						';
	...
	function hallazgos_equipo($Equipo) {
		// variables necesarias para la página
		$datos['titulo'] = "Hallazgos del Equipo Auditor";
		$datos['secciones'] = $this->Inicio_model->get_secciones();
		$datos['identidad'] = $this->Inicio_model->get_identidad();
		$datos['usuario'] = $this->Inicio_model->get_usuario();
		...
		$datos['hallazgos'] = $this->auditoria_model->get_hallazgos( $Equipo );
		...
		$this->load->view( '_estructura/header',$datos );
		$this->load->view( '_estructura/top', $datos );
		$this->load->view( 'auditoria/hallazgos_equipo', $datos );
		$this->load->view( '_estructura/right', $datos );
		$this->load->view( '_estructura/footer' );
	}
}
