            ...
            $formulario = array(
				...
				// * Boton cancelar
	                'boton_cancelar' => array (
						...
						'onclick'	=> "location.href='".base_url()."index.php/procesos/solicitudes/ver_lista_distribucion/".$id."/".$estado."/".$tipo."'",
	                ),
			);
			if( $usuarios->num_rows() > 0 ) {
				$j = 1;
				echo form_open_multipart('',array('name' => 'formulario', 'id' => 'formulario'));
				echo '<table class="tabla_form" style="width:200px; margin-left:500px"><tr><td><input type="checkbox" name="all" onclick="todos()" /></td><td>Seleccionar a todos para la lista de distribuci&oacute;n</td></tr></table><br />';
				...
				echo '</tbody></table><br />';
				echo '<div style="width:700px; text-align:center;">'.form_submit($formulario['boton'],'Aceptar');
				if( $estado != '' || $tipo != '' )
					echo form_button($formulario['boton_cancelar'],'Cancelar'); 
				echo '</div>';
				echo form_close();
			}
			else {
                echo '<table class="tabla" width="700"><tr><th width="20"><img src="'.base_url().'includes/img/icons/small/info.png" /></th><td>No se han encontrado usuarios disponibles para agregar a la lista de distribuci&oacute;n.</td></tr></table><br /><br />';
				echo '<div style="width:700px; text-align:center;">'.form_button($formulario['boton_continuar'],'Continuar').'</div>';
			}
            ?>
        </div>
    </div>