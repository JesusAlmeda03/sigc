        ...
        <div class="text">
			...
			if( $minutas->num_rows() > 0 ) {
				...
				foreach( $minutas->result() as $row ) {
					...
					echo '<th width="20"><a href="'.base_url().'index.php/procesos/minutas/comite/'.$per.'/'.$row->Ano.'" target="_blank" onmouseover="ddrivetip(\'Abrir minuta\')" onmouseout="hideddrivetip()"><img src="'.base_url().'includes/img/icons/open.gif" /></a></th>';
					echo '<td>'.$row->Periodo.'</td><td>'.$row->Ano.'</td>';
					//echo '<td><a href="'.base_url().'index.php/admin_files/minutas/acuerdos/'.$per.'/'.$row->Ano.'" onmouseover="ddrivetip(\'Agregar / editar<br />acuerdos de la reunion\')" onmouseout="hideddrivetip()"><img src="'.base_url().'includes/img/icons/edit.gif" /></a></td>';
					echo '<td><a href="'.base_url().'index.php/admin_files/minutas/puntos/'.$per.'/'.$row->Ano.'" onmouseover="ddrivetip(\'Agregar / editar<br />comentarios\')" onmouseout="hideddrivetip()"><img src="'.base_url().'includes/img/icons/edit.gif" /></a></td>';
					echo '</tr>';
				}
				echo '</tbody></table>';
				echo $sort_tabla;
